...
const BibleChapterAr: FC<Props> = ({ bookId, chapterNum }) => {
  ...
  useEffect(() => {
    const fetchChapter = async () => {
      const data = await db.getAllAsync<{ num: number, text: string }>(`SELECT versesAr.num, text FROM versesAr LEFT JOIN chapters ON versesAr.chapterId = chapters.id LEFT JOIN books ON chapters.bookId = books.id WHERE chapters.num = ? AND books.id = ?;`, [chapterNum, bookId]);
